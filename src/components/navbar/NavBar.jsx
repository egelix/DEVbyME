...
function NavBar({language, aboutRef, cvRef, contactRef, projectRef, darkMode}) {
  let links = [
  ...
  return (
    <div className="text-lg z-30 align-middle fixed top-0 left-0 shadow-md shadow-black w-full text-black bg-zinc-400 dark:text-zinc-200 dark:bg-zinc-800 transition-all duration-500 ease-in">
      <div className="md:pr-10 pl-4 py-1 pr-7 w-full md:flex justify-between items-center align-middle ">
        ...
        >
          {isOpen ? <img src={darkMode ? burgerCloseDark : burgerCloseLight} className="h-10 w-10"/> : <img src={darkMode ? burgerDark : burgerLight} className="w-10 h-7" />}
        </div>
        <ul className={`text-end pr-6 md:pr-0 bg-zinc-400 dark:bg-zinc-800 md:flex pl-9 md:pl-0 md:items-center md:justify-end md:pb-0 pb-3 absolute md:relative md:z-auto z-[-1] left-0 w-full transition-all duration-500 ease-in ${isOpen ? "inline" : "hidden"}`}>
          {links.map((link, index) => (
            <li className="font-semibold my-7 md:my-0 md:ml-8 hover:text-red-700 cursor-pointer" key={index}>
              <a onClick={() => handleLinkClick(link.link)}>{language === "english" ? link.nameEng : link.nameDe}</a>
            </li>
          ))}
        </ul>
      </div>
